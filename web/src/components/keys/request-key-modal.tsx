...
export default function RequestKeyModal({
  open,
  onClose,
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      await createRequest.mutateAsync({
        ...
        daily_cost_limit:
          Number.isFinite(dailyCents) && dailyCents > 0
            ? dailyCents
            : undefined,
      });
  ...
  return (
    <dialog className="modal modal-open">
      <div className="modal-box max-w-2xl overflow-hidden p-0">
        <div className="relative border-base-300/60 border-b bg-base-200/30 px-6 py-5">
          <button
            aria-label="Close"
          ...
          <div className="flex items-start gap-4 pr-10">
            ...
            <div>
              <h3 className="font-semibold text-xl">Request a key</h3>
              <p className="mt-2 max-w-xl text-base-content/70 text-sm leading-relaxed">
                Tell admins exactly what you are building. They will choose the
                key name and provision the org-wide key for you.
              </p>
            </div>
          </div>
        </div>
        ...
        <form className="space-y-6 px-6 py-6" onSubmit={onSubmit}>
          ...
          <div className="space-y-3 pt-1">
            ...
            <div className="grid gap-2 rounded-xl border border-warning/25 bg-warning/10 p-3 text-base-content/75 text-xs sm:grid-cols-3">
              <div className="flex items-start gap-2 rounded-lg bg-base-100/45 px-3 py-2">
                <span className="badge badge-warning badge-xs mt-0.5 shrink-0">
                  1
                </span>
                <div>
          ...
          {me?.role === "editor" ? (
            <label className="form-control w-full gap-2">
              ...
              <span className="text-base-content/60 text-xs">
                Max {dailyCostLimitFormDollars(editorMaxCents)} per day for
                editors
              </span>
            </label>
          ) : null}

          <div className="modal-action mt-2 border-base-300/60 border-t px-0 pt-5">
        ...
        {!isLoading && myRequests.length > 0 ? (
          <div className="border-base-300/60 border-t px-6 py-5">
